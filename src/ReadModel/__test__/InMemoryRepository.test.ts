...
describe('InMemoryRepository', () => {
  ...
  it('Add single read model', async () => {
    const repository = new InMemoryRepository<TestReadModel>();
    const model = new TestReadModel(new Identity('1'), 'test');
  ...
  it('Can add multiple models', async () => {
    const repository = new InMemoryRepository<TestReadModel>();
    const model1 = new TestReadModel(new Identity('1'), 'test 1');
    await repository.save(model1);
    const model2 = new TestReadModel(new Identity('2'), 'test 2');
    await repository.save(model2);
    const model3 = new TestReadModel(new Identity('3'), 'test 3');
  ...
  it('Can delete models', async () => {
    ...
    const model3 = new TestReadModel(new Identity('3'), 'test 3');
    await repository.save(model3);
    await repository.remove(new Identity('2'));
    const models = await repository.findAll();
  ...
  it('Can find by keys models', async () => {
    ...
    const model1 = new TestReadModel(new Identity('1'), 'test 1');
    await repository.save(model1);
    const model2 = new TestReadModel(new Identity('2'), 'test');
    await repository.save(model2);
    const identity3 = new Identity('3');
